...
export default function FlagDropdown() {
  const [selectedFlag, setSelectedFlag] =  useState<FlagKey>('uk');
        ...
        <Menu.Items className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          <div className="py-1">
            <Menu.Item>
              {({ active }) => (
                <button
                  onClick={() => setSelectedFlag('uk')}
                  className={`${
                    active ? 'bg-gray-100 text-gray-900' : 'text-gray-700'
                  } flex w-full items-center px-4 py-2 text-sm`}
                >
                  {flags.uk} <span className="ml-2">United Kingdom</span>
                </button>
              )}
            </Menu.Item>
            <Menu.Item>
              {({ active }) => (
                ...
                >
                  {flags.fr} <span className="ml-2">France</span>
                </button>
              )}
            </Menu.Item>
          </div>
